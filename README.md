...
全ページの閲覧は `assets/js/footer.js` から共通トラッカーへ送信する。YouTube、記事カード、ランダム表示、歌手・年代・気分フィルター、問い合わせへの遷移はクリックイベントとして区別する。

管理者の確認ブラウザを集計から除外する場合は、一度だけURL末尾に `?fga_ignore=1` を付けて開く。除外を解除する場合は `?fga_ignore=0` を付けて開く。
